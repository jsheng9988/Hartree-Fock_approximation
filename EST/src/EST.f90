...
! Electronic Structure Theory program
  ...
  !double precision,allocatable  :: ERI_MO_K(:,:,:,:)
  double precision,allocatable  :: A(:,:) ! CIS matrix
  ...
  double precision              :: start_CIS,end_CIS,t_CIS  ! CIS time
  double precision              :: start_TDHF,end_TDHF,t_TDHF  ! CIS time
...
!      = nO + nV
  ...
  call read_molecule(nAt,nEl,nO) ! (number of atoms, number of electrons ,number occupied)
  allocate(ZNuc(nAt),rAt(nAt,3))  ! Number of nuclei(number of atoms) , geometry

! Read geometry

  call read_geometry(nAt,ZNuc,rAt,ENuc) ! info atoms, number nuelci, e-, occupied things, geometry, read basis set
...
! Read integrals
  ...
  call read_integrals(nBas,S,T,V,Hc,ERI)  !Hc -h core, V is nucler nuclerar

!------------------------------------------------------------------------
! Orthogonalization X = S^(-1/2)      work here
!------------------------------------------------------------------------

  call orthogonalization_matrix(nBas,S,X) ! work here
...
!------------------------------------------------------------------------
   ! write(*,*)"ERI is:", ERI
    ...
    call AO_to_MO(nBas,c,ERI,ERI_MO)          ! work here homework
...
!------------------------------------------------------------------------
! Compute CIS energy      ! Homework here
!------------------------------------------------------------------------

    call cpu_time(start_CIS)
    call CIS(nBas,nO,ERI_MO ,e )
...
!------------------------------------------------------------------------
! Compute TDHF energy      ! Homework here
!------------------------------------------------------------------------

    call cpu_time(start_TDHF)
    call TDHF(nBas,nO,ERI_MO ,e )
